    ...
    renderPlaylist() {
        ...
        this.musicPlayer.musicLibrary.forEach((track, index) => {
            ...
            trackItem.innerHTML = `
                ...
                <div class="track-details" style="overflow: hidden; min-width: 0;">
                    <h4 style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin: 0;">${track.title || 'Unknown Track'}</h4>
                    <p style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin: 0; font-size: 0.8em; opacity: 0.8;">${track.artist || 'Unknown Artist'} • ${track.album || 'Unknown Album'}</p>
                </div>
                <div class="track-duration" style="flex-shrink: 0; margin-left: 10px;">${track.duration || '0:00'}</div>
                <button class="delete-btn" data-index="${index}" style="flex-shrink: 0;">✕</button>
            `;
    ...
    handleFiles(files) {
        ...
        for (const file of files) {
            if (file.type.startsWith('audio/')) {
                ...
                // If running in Desktop app (Electron), save natively to music folder
                if (file.path && window.electronAPI && window.electronAPI.saveMusicFile) {
                    window.electronAPI.saveMusicFile(file.path, fileName)
                        .then(newPath => {
                            this.musicPlayer.addTrackPermanent(newPath, baseName);
                            // Refresh playlist and storage counter once saved
                            this.renderPlaylist();
                            this.updateStorageUsage();
                        })
                        .catch(err => {
                            console.error('Failed to save file permanently:', err);
                            this.showMessage('Failed to save track permanently. Using temporary version.');
                            this.musicPlayer.addTrack(file, baseName);
                            this.renderPlaylist();
                        });
                    addedCount++;
                } else {
                    // Fallback to ephemeral Web Blob (for standard browsers)
                    const newTrack = this.musicPlayer.addTrack(file, baseName);
                    if (newTrack) addedCount++;
                }
            }
        }
